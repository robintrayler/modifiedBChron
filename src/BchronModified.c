...
void predictExtrapUp(double *alpha, double *lambda, double *beta, double *predictPositions, int *NpredictPositions, double *currPositions1, double *theta1, int *maxExtrap, double *extractDate, double *predvals) {
  ...
  while(bad==1) {
    ...
    if(count==50) {
      ...
      bad=0;
      warning("Unable to find suitable chronologies for top of section - truncated to upper truncation date");
    }
  }
}


void predictExtrapDown(double *alpha, double *lambda, double *beta, double *predictPositions, int *NpredictPositions, double *currPositions1, double *theta1, int *maxExtrap, double *extractDate, double *predvals) {
  // Runs the prediction code when we are extrapolating up beyond the first date
  int bad=1,count=0,i;
  double depthEvents[*maxExtrap],timeEvents[*maxExtrap];
  depthEvents[0] = *currPositions1;
  ...
  while(bad==1) {
    ...
    if(count==50) {
      ...
      bad=0;
      warning("Unable to find suitable chronologies for bottom of section - truncated to lower truncation date");
    }
  }
}
